...
class CorrelationFilterTracker(Tracker): 
    ...
    def initialize(self, image, region):
        ...
        self.size = (int(((region[2] * self.parameters.increasing_factor) // 2 * 2) + 1), int(((region[3] * self.parameters.increasing_factor) // 2 * 2) + 1))
        ...
        self.patch_gray = cv2.cvtColor(self.patch, cv2.COLOR_BGR2GRAY)
        self.f_hat = np.fft.fft2(self.patch_gray * self.cosine_filter)
        self.f_hat_conjugate = np.conj(self.f_hat)
        self.h_hat_conj = (self.gauss_filter_fft * self.f_hat_conjugate) / (self.f_hat * self.f_hat_conjugate + 0.1)
        #end = time.time()
        #print(start - end)
    ...
    def track(self, image):
        ...
        (dy, dx) = np.unravel_index(np.argmax(r, axis=None), r.shape)
        ...
        h_hat_conj_new = (self.gauss_filter_fft * f_hat_conjugate) / (f_hat * f_hat_conjugate + 0.1)

        self.h_hat_conj = ((1 - self.parameters.alpha) * self.h_hat_conj) + (self.parameters.alpha * h_hat_conj_new)
        #end = time.time();
        #print(end - start)
        if self.parameters.increasing_factor != 1:
            return [self.position[0] - self.region[2] // 2, self.position[1] - self.region[3] // 2, self.region[2], self.region[3]]
